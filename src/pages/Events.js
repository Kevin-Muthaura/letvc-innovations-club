...
export default function Events() {
  ...
  async function save() {
    ...
    try {
      if (modal==='add') {
        ...
        setEvents(e=>[...e,data].sort((a,b)=>a.event_date?.localeCompare(b.event_date)));
        await logAudit('ADD_EVENT','events',data.id,profile?.full_name,`Added: ${data.title}`);
        show('Event created!');
      } else {
        const { data } = await supabase.from('events').update(form).eq('id',modal.id).select().single();
        setEvents(e=>e.map(x=>x.id===modal.id?data:x));
        show('Event updated');
      }
  ...
  async function doDelete() {
    await supabase.from('events').delete().eq('id',delEvt.id);
    setEvents(e=>e.filter(x=>x.id!==delEvt.id));
    show('Event deleted'); setDelEvt(null);
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'1.25rem', gap:10, flexWrap:'wrap' }}>
        <div>
          <h2 style={{ fontSize:22, fontWeight:800, color:C.text }}>📅 Events & Activities</h2>
          <p style={{ color:C.text3, fontSize:13, marginTop:2 }}>{upcoming.length} upcoming · {past.length} past</p>
        </div>
      ...
      <div style={{ display:'flex', background:C.bg3, borderRadius:8, padding:4, marginBottom:'1rem', width:'fit-content' }}>
        {['upcoming','past'].map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={{ padding:'7px 20px', borderRadius:6, border:'none', cursor:'pointer', fontSize:13, fontWeight:600, fontFamily:'inherit', background:tab===t?C.surface:'transparent', color:tab===t?C.text:C.text3, transition:'all 0.15s' }}>
            {t.charAt(0).toUpperCase()+t.slice(1)} ({t==='upcoming'?upcoming.length:past.length})
          </button>
        ))}
      </div>

      {loading && <div style={{ textAlign:'center', padding:'3rem', color:C.text3 }}>Loading events…</div>}
        ...
        {displayed.map(e=>(
          <Card key={e.id} style={{ opacity:tab==='past'?0.7:1 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
              <div style={{ width:42, height:42, borderRadius:10, background:C.bg3, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <i className={`ti ti-${TYPE_ICO[e.event_type]||'calendar'}`} style={{ fontSize:20, color:C.primary2 }}/>
              </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:5, fontSize:12, color:C.text3 }}>
            ...
            {isAdmin && (
              <div style={{ marginTop:12, display:'flex', gap:6 }}>
                <Btn size="sm" icon="edit" onClick={()=>{ setForm({title:e.title,description:e.description,event_date:e.event_date||'',event_time:e.event_time||'',venue:e.venue||'',virtual_link:e.virtual_link||'',event_type:e.event_type,organizer:e.organizer,max_attendees:e.max_attendees}); setModal({id:e.id}); }}>Edit</Btn>
                <Btn size="sm" danger icon="trash" onClick={()=>setDelEvt(e)}>Delete</Btn>
              </div>
            )}
          </Card>
        ))}
      ...
      {modal && (
        <Modal title={modal==='add'?'Schedule New Event':'Edit Event'} onClose={()=>setModal(null)} width={560}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
            ...
            <Inp label="Virtual Link" value={form.virtual_link} onChange={v=>setForm({...form,virtual_link:v})} placeholder="https://meet.google.com/…" />
            ...
            <Inp label="Max Attendees" value={String(form.max_attendees)} onChange={v=>setForm({...form,max_attendees:Number(v)})} type="number" />
          </div>
