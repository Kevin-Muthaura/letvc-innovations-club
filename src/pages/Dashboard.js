...
export default function Dashboard({setPage}){
  ...
  useEffect(()=>{
    async function load(){
      const[mRes,pRes,eRes,aRes,projRes,patronRes,meetRes,ideaRes]=await Promise.all([
        ...
        supabase.from('meetings').select('id,title,meeting_date,meeting_time,venue,type,virtual_link').gte('meeting_date',new Date().toISOString().slice(0,10)).order('meeting_date').limit(2),
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:'1.5rem'}}>
      <div>
        <h2 style={{fontSize:24,fontWeight:800,color:C.text}}>{greeting}{profile?.full_name?`, ${profile.full_name.split(' ')[0]}`:''} 👋</h2>
      ...
      {data.announcements.filter(a=>a.priority==='high').map(a=>(
        <div key={a.id} style={{background:C.primaryBg,border:`1px solid ${C.primary}40`,borderRadius:10,padding:'12px 16px',display:'flex',gap:10}}>
          ...
          <div><strong style={{color:C.primary2,fontSize:14}}>{a.title}</strong><p style={{fontSize:13,color:C.text2,marginTop:3}}>{a.body}</p></div>
        </div>
      ))}
        ...
        <Card>
          ...
          {data.sections.map(({s,c})=>(
            <div key={s} style={{marginBottom:10}}>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:13,marginBottom:4}}><span style={{color:C.text2}}>{s}</span><span style={{fontWeight:700,color:C.text}}>{c}</span></div>
          ...
          <Card style={{flex:1}}>
            ...
            {data.events.map(e=>(
              <div key={e.id} style={{marginBottom:10,paddingBottom:10,borderBottom:`1px solid ${C.border}`}}>
                ...
                <div style={{fontSize:12,color:C.text3,marginTop:3}}><i className="ti ti-calendar" style={{marginRight:4}}/>{e.event_date}{e.venue&&<><span style={{margin:'0 6px'}}>·</span><i className="ti ti-map-pin" style={{marginRight:4}}/>{e.venue}</>}</div>
              </div>
            ))}
          </Card>
              ...
              {data.meetings.map(m=>(
                <div key={m.id} style={{marginBottom:8,display:'flex',gap:10,alignItems:'flex-start'}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    {m.virtual_link&&<a href={m.virtual_link} target="_blank" rel="noreferrer" style={{fontSize:11,color:C.primary2}}>Join online →</a>}
                  </div>
                </div>
              ))}
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:12}}>
          ...
          {data.projects.map(p=>(
            <div key={p.id} style={{border:`1px solid ${C.border}`,borderRadius:10,padding:'1rem',background:C.bg3}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',fontSize:12,marginBottom:4}}><span style={{color:C.text3}}>Progress</span><span style={{fontWeight:700,color:C.text}}>{p.progress}%</span></div>
