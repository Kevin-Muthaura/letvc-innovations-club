...
export default function Members({setPage}){
  ...
  function validate(){const e={};if(!form.full_name.trim())e.full_name='Required';if(!form.section)e.section='Required';setErrs(e);return!Object.keys(e).length;}
  ...
  return(
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.25rem',gap:10,flexWrap:'wrap'}}>
        <div><h2 style={{fontSize:22,fontWeight:800,color:C.text}}>Members Registry</h2><p style={{color:C.text3,fontSize:13,marginTop:2}}>{rows.length} member{rows.length!==1?'s':''} found</p></div>
      ...
      <Card style={{marginBottom:'1rem',padding:'1rem'}}>
        <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:10}}>
          ...
          <select value={fSect} onChange={e=>setFSect(e.target.value)} style={{padding:'8px 12px',background:C.bg3,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontSize:13,fontFamily:'inherit'}}>
            <option value="">All Departments</option>{SECTIONS.map(s=><option key={s}>{s}</option>)}
          </select>
          <select value={fStat} onChange={e=>setFStat(e.target.value)} style={{padding:'8px 12px',background:C.bg3,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontSize:13,fontFamily:'inherit'}}>
            <option value="">All Statuses</option>{['active','inactive','suspended'].map(s=><option key={s}>{s}</option>)}
          </select>
        </div>
      </Card>
              ...
              <tbody>
                {rows.map((m,i)=>(
                  <tr key={m.id} style={{borderTop:`1px solid ${C.border}`,background:detail?.id===m.id?C.primaryBg:'transparent',cursor:'pointer'}} onClick={()=>setDetail(detail?.id===m.id?null:m)}>
                    <td style={{padding:'12px 14px'}}><div style={{display:'flex',alignItems:'center',gap:10}}><Avatar name={m.full_name} size={34} section={m.section} url={m.avatar_url}/><span style={{fontWeight:600,fontSize:14,color:C.text}}>{m.full_name}</span></div></td>
                    ...
                    <td style={{padding:'12px 14px'}}><Badge label={m.role?.replace('_',' ')||'member'} color={ROLE_C[m.role]||'blue'}/></td>
                    <td style={{padding:'12px 14px'}}>
                      <Badge label={m.status} color={STATUS_C[m.status]} dot/>
                      {m.profile_id&&<span style={{marginLeft:6}}><Badge label="🔗" color="teal"/></span>}
                    </td>
                    {isAdmin&&(
          ...
          <Card>
            ...
            <div style={{textAlign:'center',marginBottom:16}}>
              ...
              <div style={{marginTop:8,display:'flex',gap:6,justifyContent:'center',flexWrap:'wrap'}}>
                <Badge label={detail.role?.replace('_',' ')||'member'} color={ROLE_C[detail.role]||'blue'}/>
                <Badge label={detail.status} color={STATUS_C[detail.status]} dot/>
              </div>
            </div>
            {[['Adm. No.',detail.adm_no,'id-badge'],['Phone',detail.phone,'phone'],['Email',detail.email,'mail'],['Joined',detail.joined_at,'calendar']].map(([l,v,i])=>v&&(
      ...
      {modal&&(
        <Modal title={modal==='add'?'Register New Member':'Edit Member'} onClose={()=>setModal(null)} width={560}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
            ...
            <Sel label="Department / Section" value={form.section} onChange={v=>setForm({...form,section:v})} required error={errs.section} options={[{value:'',label:'Select department…'},...SECTIONS.map(s=>({value:s,label:s}))]}/>
            <Inp label="Admission Number" value={form.adm_no} onChange={v=>setForm({...form,adm_no:v})} placeholder="e.g. DICT/M25/001"/>
            ...
            <Sel label="Role" value={form.role} onChange={v=>setForm({...form,role:v})} options={['member','club_officer','captain']}/>
            <Sel label="Status" value={form.status} onChange={v=>setForm({...form,status:v})} options={['active','inactive','suspended']}/>
          </div>
      ...
      {delRow&&<Confirm message={`Remove ${delRow.full_name}? This cannot be undone.`} onConfirm={doDelete} onCancel={()=>setDelRow(null)}/>}
    </div>
  );
}
