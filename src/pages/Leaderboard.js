...
const BADGE_DEFS = [
  { id:'pioneer',     label:'🚀 Pioneer',      desc:'First to join the club',    color:'red' },
  { id:'innovator',   label:'🧠 Innovator',    desc:'Submitted 5+ ideas',        color:'amber' },
  { id:'builder',     label:'🛠 Builder',       desc:'Part of 3+ project teams',  color:'teal' },
  { id:'top_voter',   label:'🗳️ Top Voter',    desc:'Cast 10+ votes',            color:'blue' },
  { id:'collaborator',label:'🤝 Collaborator', desc:'Joined 5+ teams',           color:'purple' },
  { id:'trending',    label:'🔥 Trending',     desc:'Had a top-voted idea',      color:'red' },
  { id:'mentor_pick', label:'⭐ Mentor Pick',  desc:'Idea approved by mentor',   color:'amber' },
];
...
export default function Leaderboard() {
  ...
  useEffect(()=>{
    async function load(){
      setLoading(true);
      // Only trainees/members on the leaderboard — exclude admin/mentor/editor
      const { data } = await supabase.from('profiles')
        .select('id,full_name,section,points,badges,role,is_active,avatar_url')
        .in('role',['member'])
        .eq('is_active',true)
  ...
  const sorted = tab==='points'
    ...
    : filtered.sort((a,b)=>(b.badges?.length||0)-(a.badges?.length||0));

  const categories = [
    { label:'🧠 Top Innovator',    sub:'Most ideas submitted',  medal:'🧠' },
    { label:'🤝 Top Collaborator', sub:'Most project teams joined', medal:'🤝' },
    { label:'🗳️ Top Voter',       sub:'Most votes cast',        medal:'🗳️' },
  ];
  ...
  return (
    <div>
      <div style={{ marginBottom:'1.25rem' }}>
        ...
        <p style={{ color:C.text3,fontSize:13,marginTop:2 }}>Trainee rankings — earn points by participating</p>
      </div>
      ...
      <div style={{ display:'flex',gap:10,marginBottom:'1rem',flexWrap:'wrap',alignItems:'center' }}>
        <div style={{ display:'flex',background:C.bg3,borderRadius:8,padding:4 }}>
          {[['points','Points'],['badges','Badges']].map(([t,l])=>(
            <button key={t} onClick={()=>setTab(t)} style={{ padding:'6px 16px',borderRadius:6,border:'none',cursor:'pointer',fontSize:13,fontWeight:600,fontFamily:'inherit',background:tab===t?C.surface:'transparent',color:tab===t?C.text:C.text3,transition:'all 0.15s' }}>{l}</button>
          ))}
        </div>
        ...
        <select value={period} onChange={e=>setPeriod(e.target.value)} style={{ padding:'7px 12px',background:C.bg3,border:`1px solid ${C.border}`,borderRadius:8,color:C.text2,fontSize:13,fontFamily:'inherit' }}>
      ...
      <Card style={{ padding:0,overflow:'hidden',marginBottom:'1.5rem' }}>
        <div style={{ padding:'12px 16px',borderBottom:`1px solid ${C.border}`,fontWeight:700,color:C.text,fontSize:14 }}>
          Full Rankings — {sorted.length} trainees
        </div>
        {loading?<div style={{ textAlign:'center',padding:'3rem',color:C.text3 }}>Loading…</div>:
        sorted.length===0?<div style={{ textAlign:'center',padding:'3rem',color:C.text3 }}>No trainees on leaderboard yet</div>:(
          sorted.map((p,i)=>(
            <div key={p.id} style={{ display:'flex',alignItems:'center',gap:14,padding:'12px 16px',borderTop:i>0?`1px solid ${C.border}`:'none',background:i<3?`${RANK_COLORS[i]}08`:'transparent' }}>
              ...
              <div style={{ display:'flex',gap:6,flexWrap:'wrap',justifyContent:'flex-end',maxWidth:200 }}>
                {(p.badges||[]).slice(0,3).map(b=>{ const bd=BADGE_DEFS.find(x=>x.id===b); return bd?<Badge key={b} label={bd.label} color={bd.color}/>:null; })}
              </div>
      ...
      <Card style={{ marginBottom:'1.5rem' }}>
        <h3 style={{ fontWeight:700,color:C.text,marginBottom:14 }}>🎖️ Category Champions</h3>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:12 }}>
          {categories.map(cat=>(
            <div key={cat.label} style={{ padding:'14px',background:C.bg3,borderRadius:10,textAlign:'center' }}>
              <div style={{ fontSize:32,marginBottom:6 }}>{cat.medal}</div>
              <div style={{ fontWeight:700,fontSize:14,color:C.text }}>{cat.label}</div>
              <div style={{ fontSize:12,color:C.text3,marginTop:3 }}>{cat.sub}</div>
              <div style={{ fontSize:13,color:C.primary2,marginTop:8 }}>
                {sorted[0]?.full_name?.split(' ')[0]||'—'}
              </div>
            </div>
          ))}
        </div>
