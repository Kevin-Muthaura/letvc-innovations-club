...
const ACT_C  = { ADD_MEMBER:'green',EDIT_MEMBER:'blue',DELETE_MEMBER:'red',ADD_PROJECT:'green',ADD_IDEA:'teal',CHANGE_ROLE:'amber',ADD_EVENT:'green',ADD_MEETING:'green',UPDATE_PATRON:'teal' };
...
export default function AdminPanel() {
  ...
  async function updateRole(profileId, role) {
    ...
    await logAudit('CHANGE_ROLE','profiles',profileId,me?.full_name,`Role → ${role}`);
  ...
  async function changePassword() {
    ...
    if (error) return show(error.message,'error');
    show('Password changed ✅'); setPwModal(false); setPwForm({ nw:'', nw2:'' });
  }
  ...
  const TABS = [
    ['users','Users & Roles','users'],['points','Points & Rules','star'],
    ['patron','Patron','user-star'],['reports','Reports','alert-triangle'],['audit','Audit Log','clipboard-list']
  ];
    ...
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'1.25rem', gap:10, flexWrap:'wrap' }}>
        <div>
          ...
          <p style={{ color:C.text3, fontSize:13, marginTop:2 }}>{profiles.length} registered users</p>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <Btn size="sm" icon="refresh" onClick={load}>Refresh</Btn>
          <Btn size="sm" icon="key" onClick={() => setPwModal(true)}>Change Password</Btn>
        </div>
      </div>
      ...
      {tab === 'users' && (
        <div>
          ...
          {loading ? <div style={{ textAlign:'center', padding:'2rem', color:C.text3 }}>Loading users…</div> : (
          <Card style={{ padding:0, overflow:'hidden', marginBottom:16 }}>
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead><tr style={{ background:C.bg3 }}>
                  {['User','Role','Points','Status','Actions'].map(h=>(
                ...
                <tbody>
                    ...
                    <tr key={p.id} style={{ borderTop:`1px solid ${C.border}`, opacity:p.is_active===false?0.5:1 }}>
                      <td style={{ padding:'12px 14px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                          ...
                          <div>
                            <div style={{ fontWeight:600, fontSize:14, color:C.text }}>{p.full_name||'(no name yet)'}</div>
                            <div style={{ fontSize:11, color:C.text3 }}>{p.email}</div>
                            {p.section&&<div style={{ fontSize:11, color:C.text3 }}>{p.section}</div>}
                          </div>
                        </div>
                      </td>
                      <td style={{ padding:'12px 14px' }}>
                      ...
                      <td style={{ padding:'12px 14px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                          <button onClick={()=>updatePoints(p.id,-10)} style={{ background:C.dangerBg, border:'none', color:C.danger, borderRadius:4, cursor:'pointer', width:22, height:22, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700 }}>−</button>
                          <span style={{ fontWeight:700, color:C.primary2, minWidth:36, textAlign:'center' }}>{p.points||0}</span>
                          <button onClick={()=>updatePoints(p.id,10)} style={{ background:C.successBg, border:'none', color:C.success, borderRadius:4, cursor:'pointer', width:22, height:22, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700 }}>+</button>
                        </div>
                      </td>
                      <td style={{ padding:'12px 14px' }}>
                  ...
                  {filtered.length===0&&<tr><td colSpan={5} style={{ padding:'2rem', textAlign:'center', color:C.text3 }}>No users found</td></tr>}
                </tbody>
              </table>
            </div>
          </Card>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:10 }}>
              {[['admin','Full access — can do everything','red'],['editor','Can add/edit all content','amber'],['mentor','Can review and approve ideas','teal'],['member','Read-only + submit ideas','blue']].map(([r,d,c])=>(
                <div key={r} style={{ padding:'10px 14px', background:C.bg3, borderRadius:8 }}>
                  <Badge label={r} color={c}/><p style={{ fontSize:12, color:C.text3, marginTop:6 }}>{d}</p>
                </div>
              ))}
            </div>
          </Card>
      ...
      {tab === 'points' && (
        <div>
          <p style={{ color:C.text3, fontSize:13, marginBottom:16 }}>Configure how many points members earn for each action.</p>
          <Card style={{ padding:0, overflow:'hidden', marginBottom:16 }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead><tr style={{ background:C.bg3 }}>
                {['Action','Description','Points','Status'].map(h=><th key={h} style={{ padding:'10px 14px', textAlign:'left', fontSize:11, fontWeight:700, color:C.text3, textTransform:'uppercase', letterSpacing:'0.5px' }}>{h}</th>)}
              </tr></thead>
              <tbody>
                {rules.map((r,i)=>(
                  <tr key={r.id} style={{ borderTop:`1px solid ${C.border}` }}>
                    ...
                    <td style={{ padding:'12px 14px' }}><Badge label={r.is_active?'Active':'Off'} color={r.is_active?'green':'gray'}/></td>
                  </tr>
                ))}
                {rules.length===0&&<tr><td colSpan={4} style={{ padding:'2rem', textAlign:'center', color:C.text3 }}>No rules found — run supabase_updates.sql first.</td></tr>}
              </tbody>
            </table>
          </Card>
          <Card>
      ...
      {tab === 'patron' && (
        <Card style={{ maxWidth:540 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <Txt label="Bio" value={pForm.bio} onChange={v=>setPForm({...pForm,bio:v})} rows={3}/>
          </div>
      ...
      {tab === 'reports' && (
        <div>
          <p style={{ color:C.text3, fontSize:13, marginBottom:16 }}>Ideas flagged by members for review.</p>
          {reports.length===0
            ...
            : reports.map(r=>(
              <Card key={r.id} style={{ marginBottom:12 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10 }}>
                  <div>
                    <div style={{ fontWeight:600, color:C.danger, fontSize:14, marginBottom:4 }}>⚠️ Reported: {r.ideas?.title||'(deleted)'}</div>
      ...
      {tab === 'audit' && (
        <Card style={{ padding:0, overflow:'hidden' }}>
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead><tr style={{ background:C.bg3 }}>
                {['Time','Action','Entity','By','Details'].map(h=><th key={h} style={{ padding:'10px 14px', textAlign:'left', fontSize:11, fontWeight:700, color:C.text3, textTransform:'uppercase', letterSpacing:'0.5px', whiteSpace:'nowrap' }}>{h}</th>)}
              </tr></thead>
              <tbody>
                {audit.map(a=>(
      ...
      {delUser&&<Confirm message={`Deactivate ${delUser.full_name}? They will lose access.`} onConfirm={()=>deactivateUser(delUser)} onCancel={()=>setDelUser(null)}/>}
    </div>
  );
}
...
function AwardBadge({ profiles, show }) {
  ...
  async function award() {
    ...
    await supabase.from('notifications').insert({ user_id:userId, message:`🏅 You were awarded the "${badge}" badge!`, type:'points' });
    show(`Badge awarded to ${p?.full_name}! 🏅`); setSaving(false);
  }
  return (
