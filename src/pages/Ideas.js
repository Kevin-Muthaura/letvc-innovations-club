...
// Lifecycle pipeline
const LIFECYCLE = ['submitted','under_review','approved','funded'];
const LC_LABEL  = { submitted:'💡 Idea', under_review:'🔍 Review', approved:'✅ Approved', funded:'🚀 Funded' };
...
export default function Ideas() {
  ...
  async function vote(idea, type) {
    ...
    if (existing===type) {
      ...
      if (existing) {
        ...
        // award points to idea author for receiving upvote
        if (type==='up' && idea.submitted_by) {
          await supabase.rpc('award_points',{ p_user_id:idea.submitted_by, p_action:'receive_upvote' });
        }
      }
    }
  }
  ...
  async function toggleSave(idea) {
    if (!profile) return;
    if (saved[idea.id]) {
      await supabase.from('saved_ideas').delete().eq('idea_id',idea.id).eq('user_id',profile.id);
      setSaved(s=>{ const n={...s}; delete n[idea.id]; return n; });
      show('Removed from saved');
    } else {
      await supabase.from('saved_ideas').insert({ idea_id:idea.id,user_id:profile.id });
      setSaved(s=>({...s,[idea.id]:true}));
      show('Idea saved 🔖');
    }
  }
  ...
  async function updateStatus(idea, status) {
    ...
    // award points if approved or funded
    ...
    // notify idea author
    if (idea.submitted_by) {
      await supabase.from('notifications').insert({ user_id:idea.submitted_by, message:`Your idea "${idea.title}" status changed to: ${status}`, type:'idea' });
    }
    show(`Status → ${status}`);
  }

  async function reportIdea(idea) {
    const reason = window.prompt('Why are you reporting this idea?');
    if (!reason) return;
    await supabase.from('idea_reports').insert({ idea_id:idea.id,reported_by:profile.id,reason });
    await supabase.from('ideas').update({ report_count:(idea.report_count||0)+1 }).eq('id',idea.id);
    show('Report submitted. Admin will review.');
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.25rem',gap:10,flexWrap:'wrap' }}>
        <div>
          <h2 style={{ fontSize:22,fontWeight:800,color:C.text }}>💡 Ideas Feed</h2>
          <p style={{ color:C.text3,fontSize:13,marginTop:2 }}>Submit, vote and discuss innovations</p>
        </div>
      ...
      <Card style={{ marginBottom:'1rem',padding:'1rem' }}>
        <div style={{ display:'grid',gridTemplateColumns:'1fr auto auto auto',gap:10,alignItems:'center' }}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Search ideas…"
            style={{ padding:'8px 12px',background:C.bg3,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontSize:13,fontFamily:'inherit',outline:'none' }}/>
          <select value={catF} onChange={e=>setCatF(e.target.value)} style={{ padding:'8px 12px',background:C.bg3,border:`1px solid ${C.border}`,borderRadius:8,color:C.text2,fontSize:13,fontFamily:'inherit' }}>
            <option value="">All Categories</option>
            {CATS.map(c=><option key={c}>{c}</option>)}
          </select>
          <select value={sort} onChange={e=>setSort(e.target.value)} style={{ padding:'8px 12px',background:C.bg3,border:`1px solid ${C.border}`,borderRadius:8,color:C.text2,fontSize:13,fontFamily:'inherit' }}>
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
        {ideas.map(idea=>{
          ...
          const isMine=idea.submitted_by===profile?.id;
          ...
          return (
            <div key={idea.id} style={{ border:`1px solid ${isOpen?C.primary:C.border}`,borderRadius:12,background:C.surface,padding:'1.25rem',transition:'border-color 0.15s' }}>
              ...
              <div style={{ display:'flex',gap:0,marginBottom:12,borderRadius:8,overflow:'hidden',border:`1px solid ${C.border}` }}>
                {LIFECYCLE.map((s,i)=>(
                  <div key={s} style={{ flex:1,padding:'4px 8px',textAlign:'center',fontSize:10,fontWeight:600,background:idea.status===s?C.primaryBg:LIFECYCLE.indexOf(idea.status)>i?C.successBg:'transparent',color:idea.status===s?C.primary2:LIFECYCLE.indexOf(idea.status)>i?C.success:C.text3,borderRight:i<3?`1px solid ${C.border}`:'none',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>
                    {LC_LABEL[s]}
                  </div>
                ))}
              </div>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:8 }}>
                    ...
                    <div style={{ display:'flex',gap:6,flexShrink:0,flexWrap:'wrap' }}>
                      ...
                      {idea.is_flagged&&<Badge label="⚠️ Flagged" color="red"/>}
                    </div>
                  </div>
                  ...
                  <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8 }}>
                    ...
                    <div style={{ display:'flex',gap:6,flexWrap:'wrap' }}>
                      ...
                      <Btn size="sm" icon="message-circle" onClick={()=>{ const n=isOpen?null:idea.id; setOpenId(n); if(n) loadComments(n); }}>
                        {isOpen?'Hide':'Comment'}
                      </Btn>
                      {/* Mentor/Admin status control */}
                      {isMentor&&(
                        <select value={idea.status} onChange={e=>updateStatus(idea,e.target.value)} style={{ padding:'4px 8px',background:C.bg3,border:`1px solid ${C.border}`,borderRadius:6,color:C.text2,fontSize:12,fontFamily:'inherit',cursor:'pointer' }}>
                      ...
                      {!isMine&&<button onClick={()=>reportIdea(idea)} style={{ background:'transparent',border:`1px solid ${C.border}`,color:C.text3,borderRadius:6,padding:'4px 8px',cursor:'pointer',fontSize:11 }}>⚠️ Report</button>}
                    </div>
                  </div>
                  ...
                  {isOpen&&(
                    <div style={{ marginTop:14,borderTop:`1px solid ${C.border}`,paddingTop:14 }}>
                      ...
                      <div style={{ display:'flex',gap:8 }}>
                        <input value={comment} onChange={e=>setComment(e.target.value)} placeholder="Add a comment… (press Enter to post)" onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&postComment(idea.id)} style={{ flex:1,padding:'8px 12px',background:C.bg3,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontSize:13,fontFamily:'inherit',outline:'none' }}/>
      ...
      {modal&&(
        <Modal title={modal==='add'?'Submit New Idea':'Edit Idea'} onClose={()=>setModal(null)} width={560}>
          ...
          <div style={{ marginTop:'1.25rem',padding:'10px 14px',background:C.primaryBg,borderRadius:8,fontSize:13,color:C.primary2 }}>
            ⭐ Submitting an idea earns you <strong>+10 points</strong> on the leaderboard!
          </div>
