...
export default function Announcements() {
  ...
  const load = useCallback(async () => {
    setLoad(true);
    const { data } = await supabase.from('announcements').select('*').order('post_date',{ascending:false}).order('id',{ascending:false});
    setItems(data || []);
    setLoad(false);
  }, []);
  ...
  async function save() {
    ...
    try {
      if (modal === 'add') {
        const { data, error } = await supabase.from('announcements').insert({ ...form, posted_by: profile?.full_name || 'Admin' }).select().single();
        if (error) throw error;
        setItems(i => [data, ...i]);
        await logAudit('ADD_ANNOUNCEMENT','announcements',data.id,profile?.full_name);
        show('Announcement posted');
      } else {
        const { data, error } = await supabase.from('announcements').update({ title:form.title, body:form.body, priority:form.priority }).eq('id',modal.id).select().single();
        if (error) throw error;
        setItems(i => i.map(x => x.id === modal.id ? data : x));
  ...
  async function doDelete() {
    await supabase.from('announcements').delete().eq('id', delItem.id);
    setItems(i => i.filter(x => x.id !== delItem.id));
    show('Announcement deleted'); setDelItem(null);
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'1.25rem', gap:10, flexWrap:'wrap' }}>
        <div>
          <h2 style={{ fontSize:22, fontWeight:800, color:C.text }}>📢 Announcements</h2>
          <p style={{ color:C.text3, fontSize:13, marginTop:2 }}>Club notices and updates</p>
        </div>
      ...
      {modal && (
        <Modal title={modal === 'add' ? 'Post Announcement' : 'Edit Announcement'} onClose={() => { setModal(null); setForm(BLANK); }} width={540}>
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <Inp label="Title" value={form.title} onChange={v => setForm({...form, title:v})} placeholder="Announcement title" required />
            <Txt label="Message" value={form.body} onChange={v => setForm({...form, body:v})} placeholder="Write your announcement here…" rows={5} required />
            <div>
              <label style={{ fontSize:11, fontWeight:700, color:C.text3, textTransform:'uppercase', letterSpacing:'0.5px', display:'block', marginBottom:8 }}>Priority</label>
              <div style={{ display:'flex', gap:8 }}>
                {['high','medium','low'].map(p => (
                  <button key={p} onClick={() => setForm({...form, priority:p})} style={{ flex:1, padding:'8px', borderRadius:8, border:`1px solid ${form.priority===p ? (p==='high'?C.danger:p==='medium'?C.warn:C.primary2) : C.border}`, background: form.priority===p ? (p==='high'?C.dangerBg:p==='medium'?C.warnBg:C.primaryBg) : 'transparent', color: form.priority===p ? (p==='high'?C.danger:p==='medium'?C.warn:C.primary2) : C.text3, cursor:'pointer', fontFamily:'inherit', fontWeight:600, fontSize:13, textTransform:'capitalize' }}>
                    {p}
                  </button>
                ))}
              </div>
