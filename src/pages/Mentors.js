...
export default function Mentors(){
  ...
  const load=useCallback(async()=>{setLoading(true);const{data}=await supabase.from('mentors').select('*').order('full_name');setMentors(data||[]);setLoading(false);},[]);
  useEffect(()=>{load();},[load]);
  ...
  async function save(){
    ...
    try{
      ...
      if(modal==='add'){const{data,error}=await supabase.from('mentors').insert(payload).select().single();if(error)throw error;setMentors(m=>[...m,data]);await logAudit('ADD_MENTOR','mentors',data.id,profile?.full_name);show('Mentor added');}
      else{const{data,error}=await supabase.from('mentors').update(payload).eq('id',modal.id).select().single();if(error)throw error;setMentors(m=>m.map(x=>x.id===modal.id?data:x));if(selected?.id===modal.id)setSelected(data);show('Mentor updated');}
      setModal(null);
    }catch(ex){show(ex.message,'error');}
    setSaving(false);
  }
  async function doDelete(){await supabase.from('mentors').delete().eq('id',delItem.id);setMentors(m=>m.filter(x=>x.id!==delItem.id));if(selected?.id===delItem.id)setSelected(null);show('Mentor removed');setDelItem(null);}
  return(
    <div>
      <Toasts/>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.25rem',gap:10,flexWrap:'wrap'}}>
        <div><h2 style={{fontSize:22,fontWeight:800,color:C.text}}>🧑‍🏫 Technical Mentors</h2><p style={{color:C.text3,fontSize:13,marginTop:2}}>{mentors.length} mentors registered</p></div>
        {isAdmin&&<Btn variant="primary" icon="user-plus" onClick={()=>{setForm(BLANK);setModal('add');}}>Add Mentor</Btn>}
      </div>
      {loading&&<div style={{textAlign:'center',padding:'3rem',color:C.text3}}>Loading…</div>}
      {!loading&&mentors.length===0&&<Empty icon="user-star" message="No mentors yet" action={isAdmin&&<Btn variant="primary" icon="user-plus" onClick={()=>{setForm(BLANK);setModal('add');}}>Add first mentor</Btn>}/>}
            ...
            <Card key={m.id} onClick={()=>setSelected(sel=>sel?.id===m.id?null:m)} highlight={selected?.id===m.id} style={{cursor:'pointer'}}>
              <div style={{display:'flex',alignItems:'flex-start',gap:12,marginBottom:10}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <Badge label={m.technical_area||'General'} color="teal"/>
                </div>
                <div style={{width:10,height:10,borderRadius:'50%',background:m.is_active?C.success:C.text3,marginTop:4,flexShrink:0}}/>
              </div>
              ...
              {isAdmin&&<div style={{marginTop:12,display:'flex',gap:6}} onClick={e=>e.stopPropagation()}>
        ...
        {selected&&(
          <Card style={{position:'sticky',top:16,alignSelf:'flex-start'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              {[['Phone',selected.phone,'phone'],['Email',selected.email,'mail']].map(([l,v,i])=>v&&(
            ...
            {isAdmin&&<div style={{display:'flex',gap:8,marginTop:14}}><Btn variant="outline" icon="edit" onClick={()=>openEdit(selected)} size="sm" style={{flex:1}}>Edit</Btn><Btn danger icon="trash" onClick={()=>setDelItem(selected)} size="sm">Remove</Btn></div>}
          </Card>
        )}
      </div>
      {modal&&(
        <Modal title={modal==='add'?'Add Technical Mentor':'Edit Mentor'} onClose={()=>setModal(null)} width={500}>
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              <div><label style={{fontSize:11,fontWeight:700,color:C.text3,textTransform:'uppercase',letterSpacing:'0.5px',display:'block',marginBottom:5}}>Technical Area</label>
            ...
            <Txt label="Bio / Expertise" value={form.bio} onChange={v=>setForm({...form,bio:v})} rows={3}/>
          </div>
      ...
      {delItem&&<Confirm message={`Remove ${delItem.full_name}?`} onConfirm={doDelete} onCancel={()=>setDelItem(null)}/>}
    </div>
  );
}
