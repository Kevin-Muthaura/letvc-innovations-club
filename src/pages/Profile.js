...
export default function Profile() {
  ...
  function startEdit() {
    setForm({ full_name:profile?.full_name||'', section:profile?.section||'', phone:profile?.phone||'', bio:profile?.bio||'', skills:profile?.skills||[], year_of_study:profile?.year_of_study||'' });
    setEditing(true);
  }
  ...
  async function uploadPhoto(e) {
    ...
    try {
      ...
      const { error: dbErr } = await supabase.from('profiles').update({ avatar_url: publicUrl+'?t='+Date.now() }).eq('id',profile.id);
      ...
      reader.onload = async ev => {
        await supabase.from('profiles').update({ avatar_url: ev.target.result }).eq('id',profile.id);
        await refreshProfile();
        show('Profile photo updated! 📸');
        setUploading(false);
      };
  ...
  async function save() {
    setSaving(true);
    const { error } = await supabase.from('profiles').update({ full_name:form.full_name, section:form.section, phone:form.phone, bio:form.bio, skills:form.skills, year_of_study:form.year_of_study }).eq('id',profile.id);
  ...
  return (
    <div style={{ maxWidth:700,margin:'0 auto' }}>
      ...
      <Card style={{ marginBottom:16 }}>
        <div style={{ display:'flex',gap:18,alignItems:'flex-start',flexWrap:'wrap' }}>
          ...
          <div style={{ position:'relative',flexShrink:0 }}>
            <Avatar name={profile.full_name||profile.email||'?'} size={88} section={profile.section} url={profile.avatar_url}/>
          ...
          <div style={{ flex:1 }}>
            ...
            {profile.year_of_study&&<div style={{ fontSize:13,color:C.text3,marginTop:2 }}>{profile.year_of_study}</div>}
      ...
      <Card style={{ marginBottom:16 }}>
        ...
        {[['Phone',profile.phone,'phone'],['Section',profile.section,'building'],['Year',profile.year_of_study,'school'],['Admission No.',profile.adm_no,'id-badge']].map(([l,v,i])=>v&&(
      ...
      <Card>
        ...
        {(profile.skills||[]).length===0
          ?<p style={{ color:C.text3,fontSize:13 }}>No skills added yet. Click Edit Profile to add skills.</p>
