...
import { Avatar, Btn, Empty, useToast, C } from '../components/UI';
...
export default function Messages() {
  ...
  const loadContacts = useCallback(async () => {
    ...
    const um = {}; (unread || []).forEach(m => { um[m.sender_id] = (um[m.sender_id] || 0) + 1; });
    setUnreadMap(um);
    setLoadingC(false);
  }, [profile.id]);
  ...
  function backToList() { setActive(null); }
  ...
  return (
    <div style={{ height:'calc(100vh - 120px)', display:'flex', flexDirection:'column' }}>
      ...
      <div className="msg-shell" style={{ flex:1, display:'flex', border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden', background:C.surface, minHeight:0 }}>
        ...
        <div className="msg-contacts" style={{ width:300, minWidth:300, borderRight:`1px solid ${C.border}`, display:'flex', flexDirection:'column' }}>
          <div style={{ padding:'10px', borderBottom:`1px solid ${C.border}` }}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Search members…"
              style={{ width:'100%', padding:'8px 12px', background:C.bg3, border:`1px solid ${C.border}`, borderRadius:8, color:C.text, fontSize:14, fontFamily:'inherit', outline:'none', boxSizing:'border-box' }}/>
          </div>
          <div style={{ flex:1, overflowY:'auto' }}>
            ...
            {filtered.map(c => (
              <div key={c.id} onClick={()=>openContact(c)}
                style={{ display:'flex', alignItems:'center', gap:10, padding:'12px', cursor:'pointer', background:active?.id===c.id?C.primaryBg:'transparent', borderBottom:`1px solid ${C.border}`, transition:'background 0.15s' }}>
                <Avatar name={c.full_name||c.email} size={38} section={c.section} url={c.avatar_url}/>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:600, fontSize:14, color:C.text, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{c.full_name||c.email}</div>
                  <div style={{ fontSize:12, color:C.text3 }}>{c.section||c.role}</div>
                </div>
          ...
          {!active ? (
            ...
            <>
              <div style={{ padding:'12px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', gap:10 }}>
                <button onClick={backToList} className="msg-back-btn"
                  style={{ display:'none', background:'none', border:'none', cursor:'pointer', color:C.text2, padding:'4px 8px 4px 0', alignItems:'center' }}>
                  <i className="ti ti-arrow-left" style={{ fontSize:22 }}/>
                </button>
                <Avatar name={active.full_name||active.email} size={36} section={active.section} url={active.avatar_url}/>
                <div style={{ minWidth:0, flex:1 }}>
                  <div style={{ fontWeight:700, fontSize:15, color:C.text, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{active.full_name||active.email}</div>
                  <div style={{ fontSize:12, color:C.text3 }}>{active.section}</div>
                </div>
              </div>
                ...
                {thread.map(m => {
                  ...
                  return (
                    <div key={m.id} style={{ display:'flex', justifyContent:mine?'flex-end':'flex-start' }}>
                      <div style={{ maxWidth:'78%', background:mine?C.primary:C.bg3, color:mine?'#fff':C.text, borderRadius:12, padding:'9px 14px', fontSize:14, lineHeight:1.5, wordBreak:'break-word' }}>
                        {m.content}
              ...
              <div style={{ padding:'12px', borderTop:`1px solid ${C.border}`, display:'flex', gap:8 }}>
                <input value={text} onChange={e=>setText(e.target.value)} placeholder="Type a message…" onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&send()}
                  style={{ flex:1, minWidth:0, padding:'11px 14px', background:C.bg3, border:`1px solid ${C.border}`, borderRadius:8, color:C.text, fontSize:15, fontFamily:'inherit', outline:'none' }}/>
      ...
      <style>{`
        @media (max-width: 767px) {
          ...
          .msg-shell    { border-left: none !important; border-right: none !important; border-radius: ${active ? '0' : '12px'} !important; }
        }
      `}</style>
    </div>
  );
